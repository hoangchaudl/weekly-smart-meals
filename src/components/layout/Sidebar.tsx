...
import { UserMenu } from "@/components/UserMenu"; // <--- 1. Import this
...
export function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 h-screen w-20 bg-card shadow-card border-r border-border flex flex-col items-center py-6">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Tooltip on hover */}
              <div className="absolute left-full ml-3 px-3 py-1.5 bg-foreground text-background text-sm font-medium rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-50">
                {item.label}
              </div>
            </NavLink>
          );
      ...
      {/* Bottom User Profile (Replaced the salad icon) */}
